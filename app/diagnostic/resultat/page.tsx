...
function ResultInner() {
  ...
  useEffect(() => {
    (async () => {
      ...
      // Test frais : réponses + prénom saisis au début, compte connecté.
        ...
        try {
          ...
          if (sess.session) {
            const res = await fetch("/api/persons", {
              ...
              body: JSON.stringify({ name: ident?.name ?? "", lang, selfScores: scores }),
            });
            const j = await res.json();
            if (j.id) {
              setPersonId(j.id);
    ...
    return (
      <main className="fl-wrap" style={{ minHeight: "100vh" }}>
        ...
        <div style={{ maxWidth: 560, margin: "16vh auto 0", textAlign: "center" }}>
          ...
          <Link href="/diagnostic" className="fl-btn fl-reveal" style={{ textDecoration: "none", display: "inline-block", marginTop: 26 }}>
            {tr("landingCta", lang)}
          </Link>
        </div>
      </main>
  ...
  if (!model || !revealed) {
    return (
      <main className="fl-wrap" style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <p style={{ fontFamily: DISPLAY, fontStyle: "italic", color: C.muted, fontSize: 18 }}>{tr("computing", lang)}</p>
      </main>
    );
  }
  ...
  return (
    <main className="fl-wrap" style={{ minHeight: "100vh" }}>
      ...
      <h1 style={{ fontFamily: DISPLAY, fontSize: "clamp(40px,7vw,74px)", lineHeight: 1, margin: 0, fontWeight: 400, letterSpacing: "-0.01em" }}>
      ...
      <div style={{ marginTop: 40 }}>
            ...
            {model.scores.map((s, i) => {
              ...
              return (
                <div key={s.forceId}>
                  ...
                  <p style={{ fontSize: 12.5, lineHeight: 1.45, color: C.muted, marginTop: 7 }}>{copy?.description[lang]}</p>
                </div>
              );
            })}
          </div>
        ...
        <div style={{ textAlign: "center", marginTop: 48, display: "flex", gap: 22, justifyContent: "center", alignItems: "center", flexWrap: "wrap" }}>
          <Link href={personId ? `/360?id=${personId}` : "/360"} className="fl-btn fl-reveal" style={{ textDecoration: "none", display: "inline-block" }}>
            {tr("ask360", lang)}
          </Link>
        ...
        <p style={{ marginTop: 40, fontSize: 12.5, lineHeight: 1.6, color: "rgba(154,147,168,.8)", textAlign: "center", maxWidth: 620, marginLeft: "auto", marginRight: "auto" }}>
